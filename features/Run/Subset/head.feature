Feature:
  ...
  Scenario: We want only the first records (head strategy) from the table
    ...
    And the table posts in target database should have 2 row
    And the table posts in target database should contain rows:
      | id | title   |
      | 1  | title 1 |
      | 2  | title 2 |

  Scenario: We want only the first records (head strategy) from the table (multiple chunks)
    Given the config test.yaml contains:
    """
    tables:
      posts:
